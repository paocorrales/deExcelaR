...
Uff... pero ponerle el punto de miles a los años queda raro. La solucíon es convertir la columna `anio` en caracter. De esta manera, no se ve afectada por el formato numérico. 
...
El primer argumetno es el número de la columna que define el grupo. En este caso, la columna `pais` es la primera. El segundo argumento es la alineación vertical. Por defecto, `collapse_rows` pone las etiquetas en el centro, pero la convención más general es ponerlas arriba. 

¿Qué es esa `kable_styling()`? {kableExtra} permite cambiar el estilo de las tablas muy fácilmente. `kable_styling` es el estilo por defecto, que es igual al que produce `kable`, pero tiene muchos otros. Si viste tablas hechas en LaTeX, quizás est estilo te resulte familiar:
...
Lo notable de `column_spec` es que en sus argumentos se pueden poner vectores de manera de generar formato condicional. Para resaltar únicamente el país "Argentina"
