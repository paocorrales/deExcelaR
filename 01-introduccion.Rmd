...
La "flechita" `<-` es el operador de asignación, que le dice a R que tome el resulado de la derecha y lo guarde en una variable con el nombre que está a la izquierda. Vas a ver que no te devuele el resultado. Para verlo, ejecutamos
...
te instala un paquete que contiene muchas funciones para el manejo de texto. Luego, usando el comando

```{r}
library(stringi)
```

le decís a R que cargue las funciones que vienen en el paquete stringi para usarlas.


::: {.alert .alert-success}
**Nota**: Si cerrás y volveś a abrir R, vas a tener que usar `library(stringi)` nuevamente para acceder a la funcionalidad del paquete stringi.
:::
