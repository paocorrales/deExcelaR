...
Para generar el archivo de salida el paquete `knit` (y algunos otros) que en inglés significa *tejer* ejecutará el código en una sesión independiente de R e interpretará el teto, su formato y cualquier otra cosa que agreguemos (por ejemplo imágenes o links externos). Esto significa que nuestro archivo debe tener **todo** lo necesario para generar el análisis y si nos olvidamos de algo va a dar error. 
...
Podés encontrar este archivo de ejemplo [acá](mi-primer-rmarkdown.Rmd).
...
Y tambień podés agregar ecuaciones en LaTeX en la misma línea `$E = mc^2$` en un línea idependiente:
