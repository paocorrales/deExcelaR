...
Habrás notado un problema. El texto en la columnas `variable` toavía tiene un "_" al final. Podrías usar `mutate()` y un poco de funciones de manipulación de caracteres para quitarlo, pero hay una forma un poco más simple y es separando la columna `variable_anio` en tres, incluyendo una columna con el guión:
...
Ahora vamos a ver como unir tablas de datos a partir de una variable en común o *key*. En Excel o cualquier hoja de cálculo resolvemos esto con las funciones `VLOOKUP`, en R y en particular dentro del mundo de dplyr vamos a usar las funciones `*_join()` que justamente *unen* tablas.

Estas funciones son una familia, hay una por cada tipo de unión que querramos hacer. Vamos a ver algunas, asumiendo que queremos unir dos data.frames o tablas `x` e `y` que tienen en común una variable `A`:

![](img/join.png)

* `full_join()`: devuelve todas las filas y todas las columnas de ambas tablas `x` e `y`. Cuando no coinciden los elementos en `x1`, devuelve `NA`. Esto significa que no se pierden filas de ninguna de las dos tablas aún cuando no hay coincidencia. Está es la manera más segura de unir tablas.

* `left_join()`: devuelve todas las filas de `x` y todas las columnas de `x` e `y`. Las filas en `x` que no tengan coincidencia con `y` tendran `NA` en las nuevas columnas. Si hay multiples coincidencias entre `x`e `y`, devuelve todas las coincidencias posibles. 

* `inner_join()`: devuelve todas las filas de `x` donde hay coincidencias con `y` y todas las columnas de `x` e `y`. Si hay multiples coincidencias entre `x` e `y`, entonces devuelve todas las coincidencias. Esto significa que eliminará las filas (observaciones) que no coincidan en ambas tablas, lo que puede ser peligroso.

![](img/join_family.png)

Ahora vamos a seguir trabajando con las base de datos de `paises` pero nos vamos a quedar solo con las observaciones del 2007 y de paso unirlo a una nueva base de datos `co2` que contiene información de la emisión de dióxido de carbono de cada pais para ese mismo año. 
...
En esta nueva tabla tenemos 3 columnas, el `codigo_iso` de cada país (abreviaturas que se usan internacionalmente), `emision_co2`y `pais`. Esta última columna también está presente en la tabla paises_2007 y es la que nos servrá como variable llave para unir las dos tablas. 
...
* que variable o variables (se puede usar más de una!) usar para identificar coincidencias: el argumento `by`.
...
* Todas las observaciones están presentes, aún los paises que están presentes en `co2_2007` pero no en `paises_2007` y viseversa. En esos casos ahora tenemos `NA`. Esto genera una tabla con 241 filas.
...
Ahora esperamos que la tabla resultante tenga la misma cantidad de filas que `paises_2007` y efectivamente eso ocurre. Pero al mismo tiempo varios paises en esa tabla no encontraron coincidencia en `co2_2007` y por esa razón, la columna nueva columna `emisiones_co2` tiene `NA`.

Finalmente, si quiseramos quedarnos solo con las observaciones, en este caso paises, que están presentes en ambas tablas usamos `inner_join()`.
...
Estuvimos trabajando con una parte de la base de datos de emisiones. Pero también está disponible `co2_completo.csv` que contiene las emisiones para distintos años. El objetivo es que unas `paises` y `co2` teniendo en cuenta tanto el pais como el año. Para eso:
...
3. Uní las dos tablas usando `full_join()`, tené en cuenta que ahora usamos dos variables llave `pais` y `anio`. Buscá en la documentación cómo indicarle eso a la función join. 
:::
